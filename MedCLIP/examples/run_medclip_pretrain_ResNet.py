...
train_dataset = RSNASuperviseImageDataset(['train'], class_names=class_names, imgtransform=transform)
val_dataset = RSNASuperviseImageDataset(['val'], class_names=class_names, imgtransform=transform)
...
model = MedCLIPModel(vision_cls=MedCLIPVisionModel, device=torch.device("cpu")) #use ViT #use CPU
model.to(device)
...
model_save_path = './checkpoints/rsna_binary_classification_SU_ResNet'
...
#########-----------------TESTING-----------------#########
test_dataset = RSNASuperviseImageDataset(['test'], class_names=class_names, imgtransform=transform)
